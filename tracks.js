...
d3.json(`https://raw.githubusercontent.com/JBreitenbr/Julias-Charts-Data/refs/heads/main/${period.split(" ")[1]}/${period.split(" ")[0]}/tracks.json`).then(data => {
...
.duration(200)
.style("opacity", 0.9);
tooltip.html(`Track: ${d.track}<br/>Album: ${d.album_name}<br/>Duration: ${d.dur_min} min<br/>Score: ${Math.round(100*d.track_score)/100}<br/>Times played: ${d.plays}<br/>Played on: ${d.track_unique_days} days`)
